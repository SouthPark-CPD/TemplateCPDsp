...
(function(){
 ...
 function start(){
  ...
  }else if(!embedded) fetch('/api/auth/session',{credentials:'same-origin',cache:'no-store'}).then(r=>r.ok?r.json():null).then(d=>{const n=d?.user?.globalName||d?.user?.username;if(n)document.querySelector('#tablet-name').textContent=n;}).catch(()=>{});
 }
