...
async function load(){
  try{const [templateData,agentData]=await Promise.all([api("/api/academy-admin-data/training-templates"),api("/api/academy-admin-data/agents")]);templates=templateData.templates;agents=agentData.agents;els.instructor.textContent=agentData.instructor?.globalName||agentData.instructor?.username||"Instructeur";refreshAgentSelect();const requestedAgent=new URLSearchParams(location.search).get("agent");if(requestedAgent&&agents.some(agent=>agent.discordId===requestedAgent))els.agent.value=requestedAgent;renderLibrary();els.message.hidden=true;els.content.hidden=false;}catch(error){if(error.message!=="unauthorized")els.message.textContent=error.message==="templates_table_missing"?"Les tables des grilles ne sont pas encore installées dans Neon.":"Impossible de charger le centre de formation.";}
}
