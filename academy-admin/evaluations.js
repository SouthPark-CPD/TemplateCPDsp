...
async function load(){
  try{const [templateData,agentData]=await Promise.all([api("/api/academy-admin-data/training-templates"),api("/api/academy-admin-data/agents")]);templates=templateData.templates;agents=agentData.agents;els.instructor.textContent=agentData.instructor?.globalName||agentData.instructor?.username||"Instructeur";refreshAgentSelect();const params=new URLSearchParams(location.search);const requestedAgent=params.get("agent");if(requestedAgent&&agents.some(agent=>agent.discordId===requestedAgent))els.agent.value=requestedAgent;if(params.get("date"))els.date.value=params.get("date");renderLibrary();if(params.get("template")&&templates.some(template=>template.id===params.get("template")&&template.active)){els.template.value=params.get("template");renderEvaluation();}els.message.hidden=true;els.content.hidden=false;}catch(error){if(error.message!=="unauthorized")els.message.textContent=error.message==="templates_table_missing"?"Les tables des grilles ne sont pas encore installées dans Neon.":"Impossible de charger le centre de formation.";}
}

els.newTemplate.addEventListener("click",()=>{window.openFormationTab?.("templates");openEditor();});els.addCriterion.addEventListener("click",()=>addCriterion());els.closeEditor.addEventListener("click",()=>els.dialog.close());els.cancelEditor.addEventListener("click",()=>els.dialog.close());
...
els.form.addEventListener("submit",async event=>{event.preventDefault();const template=currentTemplate();if(!template||!els.agent.value)return;const rows=evaluationRows();const calc=calculate(rows);const strengths=rows.filter(r=>r.rating==="acquis").map(r=>r.label).join(" · ").slice(0,3000);const improvements=rows.filter(r=>r.rating==="partiel"||r.rating==="non_acquis").map(r=>`${r.label}${r.note?` (${r.note})`:""}`).join(" · ").slice(0,3000);const evaluationData={templateId:template.id,templateName:template.name,score:calc.score,suggestedResult:calc.suggested,criteria:rows.map(r=>({criterionId:r.id,section:r.section,label:r.label,rating:r.rating,weight:r.weight,critical:r.critical,note:r.note}))};try{const scheduleId=new URLSearchParams(location.search).get("schedule")||undefined;await api("/api/academy-admin-data/training/create",{method:"POST",body:JSON.stringify({scheduleId,discordId:els.agent.value,trainingType:template.name,trainingDate:els.date.value,result:els.result.value,score:calc.score,comment:els.comment.value.trim(),strengths,improvements,evaluationData})});localStorage.removeItem(DRAFT_KEY);showNotice("L’évaluation a été enregistrée dans le dossier de l’agent.");els.form.reset();els.date.valueAsDate=new Date();els.template.value="";els.criteriaSheet.innerHTML="";els.sheet.hidden=true;els.empty.hidden=false;resultWasManuallyChanged=false;}catch(error){const message=error.message==="evaluation_template_unavailable"?"Cette grille a été modifiée ou désactivée. Rechargez la page.":"Impossible d’enregistrer l’évaluation.";showNotice(message,"error");}});
