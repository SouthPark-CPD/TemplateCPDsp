...
let liaisonMentionCache = { expires: 0, members: [] };
let cpdDisplayNameCache = { expires: 0, names: new Map() };
...
async function enrichDiscordMessages(messages) {
  if (!Array.isArray(messages) || !messages.length) return Array.isArray(messages) ? messages : [];
  const authorIds = [...new Set(messages.map(message => String(message?.author?.id || "")).filter(validDiscordId))];
  if (!authorIds.length) return messages;
  try {
    let names = cpdDisplayNameCache.names;
    if (cpdDisplayNameCache.expires <= Date.now() || !names.size) {
      const members = await getAllCpdMembers();
      names = new Map(members.filter(member => member?.user?.id).map(member => [
        String(member.user.id),
        member.nick || member.user.global_name || member.user.username || ""
      ]).filter(([, name]) => name));
      cpdDisplayNameCache = { expires: Date.now() + 60_000, names };
    }
    return messages.map(message => {
      const botAuthorLabel = message?.author?.bot
        ? String(message.content || "").match(/^\*\*([^*\n]{1,100})\*\*/)?.[1]
        : "";
      const displayName = botAuthorLabel || names.get(String(message?.author?.id || ""));
      if (!displayName || !message.author) return message;
      return { ...message, author: { ...message.author, displayName } };
    });
  } catch (error) {
    console.warn("Discord member display names unavailable", error.status || error.message);
    return messages;
  }
}
...
async function resolveCpdDisplayName(discordId, fallback = "Agent LAPD") {
  const id = String(discordId || "");
  if (id && cpdDisplayNameCache.expires > Date.now() && cpdDisplayNameCache.names.has(id)) {
    return cpdDisplayNameCache.names.get(id);
  }
  if (!validDiscordId(id)) return fallback;
  try {
    const member = await getCpdMember(id);
    const name = member?.nick || member?.user?.global_name || member?.user?.username || fallback;
    const names = cpdDisplayNameCache.names instanceof Map ? cpdDisplayNameCache.names : new Map();
    names.set(id, name);
    cpdDisplayNameCache = { expires: Date.now() + 60_000, names };
    return name;
  } catch (error) {
...
function normalizeMentionIds(value, max = 12) {
  return [...new Set((Array.isArray(value) ? value : []).map(item => String(item || "")).filter(validDiscordId))].slice(0, max);
}

function mentionMarkup(ids) {
  return normalizeMentionIds(ids).map(id => `<@${id}>`).join(" ");
}
...
async function createGovernmentComplaint(req, res) {
  ...
  const author = await resolveCpdDisplayName(access.session.user?.id, access.session.user?.globalName || access.session.user?.username || "Agent LAPD");
  const content = [
    `## ${complaintId} — ${subject}`,
    ...
    `**Témoins :** ${witnesses}`,
    safeMentions.length ? `**Agents notifiés :** ${mentionMarkup(safeMentions)}` : "",
    "",
  ...
  try {
    const thread = await discordBotRequest(`/channels/${forumId}/threads`, {
      method: "POST",
      body: {
...
async function governmentComplaints(req, res, forumKind = "complaints") {
  ...
  let stage="forum";
  try {
    let forum={available_tags:[]}; try { forum=await discordBotRequest(`/channels/${forumId}`); } catch(e) { console.warn("Government forum metadata unavailable",e.status||e.message); }
    stage="active"; const active = await discordBotRequest(`/guilds/${guildId}/threads/active`);
...
async function liaisonMentionCandidates(req, res) {
  if (req.method !== "GET") return res.status(405).end();
  res.setHeader("Cache-Control", "private, no-store");
  const access = await validatePoliceSession(req, false);
  if (!access.ok) return res.status(401).json({ ok: false, code: access.reason });
  try {
    if (liaisonMentionCache.expires > Date.now() && liaisonMentionCache.members.length) {
      return res.json({ ok: true, members: liaisonMentionCache.members });
    }
    const members = await getAllCpdMembers();
    const candidates = members
      ...
      .filter(member => validDiscordId(member.id))
      .sort((a, b) => a.name.localeCompare(b.name, "fr", { sensitivity: "base" }));
    liaisonMentionCache = { expires: Date.now() + 60_000, members: candidates };
    return res.json({ ok: true, members: candidates });
  } catch (error) {
    console.error("Liaison mention candidates failed", { status: error.status || 0 });
    return res.status(error.status === 403 ? 403 : 502).json({ ok: false, code: error.status === 403 ? "members_forbidden" : "discord_unavailable" });
  }
}
...
async function liaisonChannelSend(req, res) {
  ...
  try { await discordBotRequest(`/channels/${channelId}`);const agent=await resolveCpdDisplayName(access.session.user?.id,access.session.user?.globalName||access.session.user?.username||"Agent LAPD");const text=`**${complaintText(agent,100)}**${safeMentions.length?`\n${mentionMarkup(safeMentions)}`:""}${safeContent?`\n${safeContent}`:""}`;const message=safeFiles.length?await discordBotUpload(`/channels/${channelId}/messages`,text,safeFiles,safeMentions,limits):await discordBotRequest(`/channels/${channelId}/messages`,{method:"POST",body:{content:text,allowed_mentions:{parse:[],users:safeMentions}}});return res.status(201).json({ok:true,messageId:message?.id||null}); }
...
async function adminDiscordDiagnostic(req, res) {
  if (req.method !== "POST") return res.status(405).end();
  res.setHeader("Cache-Control", "private, no-store");
  const admin = await configurationAdminAccess(req, res);
  if (!admin) return;
  const body = readJsonBody(req) || {};
  const config = sanitizeConfig(body.config || admin.current.value);
  const started = Date.now();
  const servers = [];
  const channels = [];
  for (const server of config.servers) {
    try {
      const guild = await discordBotRequest(`/guilds/${server.guildId}`);
      servers.push({ key: server.key, id: server.guildId, label: server.label, ok: true, discordName: guild.name || server.label, status: 200 });
    } catch (error) {
      servers.push({ key: server.key, id: server.guildId, label: server.label, ok: false, status: error.status || 0, code: error.status === 404 ? "serveur_inaccessible" : "discord_unavailable" });
    }
  }
  for (const channel of config.liaison.channels) {
    const server = serverByKey(config, channel.guildKey);
    try {
      const discordChannel = await discordBotRequest(`/channels/${channel.channelId}`);
      let readStatus = "non_testé";
      if (channel.canRead) {
        if (channel.type === "forum") {
          await discordBotRequest(`/guilds/${server.guildId}/threads/active`);
          await discordBotRequest(`/channels/${channel.channelId}/threads/archived/public?limit=1`);
        } else {
          await discordBotRequest(`/channels/${channel.channelId}/messages?limit=1`);
        }
        readStatus = "ok";
      }
      channels.push({ key: channel.key, id: channel.channelId, label: channel.label, type: channel.type, ok: true, status: 200, discordName: discordChannel.name || channel.label, readStatus });
    } catch (error) {
      channels.push({ key: channel.key, id: channel.channelId, label: channel.label, type: channel.type, ok: false, status: error.status || 0, code: error.status === 403 ? "permission_refusée" : error.status === 404 ? "canal_introuvable" : "discord_unavailable" });
    }
  }
  return res.status(200).json({ ok: true, checkedAt: new Date().toISOString(), durationMs: Date.now() - started, botConfigured: Boolean(process.env.DISCORD_BOT_TOKEN), databaseConfigured: Boolean(process.env.DATABASE_URL), servers, channels });
}
