...
(()=>{
 ...
 const mdt=[['procedures','Procédures','book'],['radio','Radio','radio'],['reglement','Règlement','list'],['tenues','Tenues','users'],['organigramme','Organigramme','chart'],['rapide','Accès rapide','grid']];
 ...
 function renderLinks(id,items){document.getElementById(id).innerHTML=items.map(([key,label,k])=>`<a href="${R.shellUrl(R.resolve(R.routes[key],origin),origin)}" data-view="${key}">${icon(k)}<span>${label}</span></a>`).join('');}
 ...
 async function session(){
  ...
  try{
   ...
   if(authorized){renderLinks('pa-links',pa);const count=Number(data.academySummary?.pendingCount||0);const badge=document.getElementById('pa-count');badge.textContent=String(count);badge.hidden=count<=0;}
