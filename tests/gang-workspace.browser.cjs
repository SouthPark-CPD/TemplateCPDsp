...
(async () => {
 ...
 await page.route('http://cpd.test/api/liaison/complaints*',route=>route.fulfill({json:{ok:true,threads:[],liaison:{navigation:{sections:{academy:{label:'PA <img src=x onerror="window.labelInjected=true">',enabled:true}}}}}}));
 await page.goto('http://cpd.test/mdt/index.html');await page.locator('[data-section="academy"] .app-badge').waitFor();assert.equal(await page.locator('#shell-message').isVisible(),false);assert.equal(await page.locator('[data-section="academy"] img').count(),0);assert.equal(await page.evaluate(()=>window.labelInjected),undefined);await page.locator('[data-section="academy"]').click();assert.equal(await page.locator('[data-view="recrutements"] .app-badge').textContent(),'3');
 ...
 await page.addStyleTag({path:path.join(__dirname,'../assets/motion.css')});await page.emulateMedia({reducedMotion:'reduce'});assert.equal(await page.locator('.case-file-body').evaluate(el=>getComputedStyle(el).animationName),'none');
 assert.deepEqual(errors,[]);console.log('PASS: dossiers, observations, repère au zoom maximal, accueil, badges, tickets, libellés protégés et réduction des animations.');
